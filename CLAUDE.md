...
**`asd attach` 是 VT 渲染客户端（2026-07-14，对标 boo 的 `boo ui`）**，不是哑管道：客户端自带一份 `GhosttyVt`（asd-cli 因此依赖 asd-vt），把 daemon 的 Snapshot+Output 喂进去、自己渲染（`render.rs`：RenderSnapshot→ANSI）。本地 VT 模型让 live 视图有：① 交替屏（`1049h`）detach 恢复原屏；② 滚回历史（滚自己的视口 `set_scroll`，键盘 `Shift+PageUp/PageDown/Home/End` 驱动 `parse_scroll_key`，交替屏上宿主原生 scrollback 不可用；**客户端本地、不影响其他 attach 的人**）。

**鼠标：镜像 session 模式（2026-07-14 定稿）**。客户端不一刀切开/关鼠标上报，而是**跟随 session 自身的鼠标模式动态开关**：`VtBackend::mouse_modes()` 读出 session 当前开着的 DEC 模式（tracking `9/1000/1002/1003` + encoding `1005/1006/1015/1016`），每次 `feed` 后 `sync_host_mouse` 用 `mouse_mode_delta`（纯函数，只发增量 `?Nh/?Nl`）镜像到宿主。**要点**：session 没鼠标（shell 提示符）时宿主也没鼠标 → **原生拖选/复制可用**；session 要鼠标（vim/htop）时宿主开对应模式 → 鼠标事件 `is_mouse_report` 到达后，live 视图（`scroll==0`）**原样透传**给 session（因为宿主镜像了 session 的确切编码，宿主发的就是 session 期望的编码，坐标 1:1，无需转换），scrollback（`scroll>0`）时鼠标走本地（`mouse_wheel` 滚动、其余吞掉、都不转发）。多人 attach：每客户端各自 vt+host_mouse 状态，天然隔离。渲染要点：`render_frame` 只在 `cursor.position` 有值时才 `?25h`（滚出视口 position=None 时绝不显示，否则右下角留光标残影）。
